...
async function toggle() {
    if (!terminal) {
        terminal = await coc_nvim_1.workspace.createTerminal({ name: 'coc-terminal' });
        if (!terminal) {
            coc_nvim_1.workspace.showMessage(`Create terminal failed`, 'error');
            return;
        }
    }
    if (showing) {
...
async function repl() {
    ...
    if (!replAvailableLanguages.includes(filetype.toString())) {
        coc_nvim_1.workspace.showMessage(`No REPL support for ${filetype} yet`, 'warning');
        return;
    }
    let prog = '';
